...
void InitKeypad(void){
  ...
  GPIOIntDisable(GPIO_PORTE_BASE,  GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4);
  
  
  GPIOIntClear(GPIO_PORTE_BASE,  GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4); 
  GPIOIntRegister(GPIO_PORTE_BASE, KeyPressedHandler);       // 23mel handler 
  GPIOIntTypeSet(GPIO_PORTE_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3|GPIO_PIN_4, GPIO_RISING_EDGE);
...
void ChangeHandler(uint8_t Pin)
{
  if(isPressedFlag == true)
  {
    
    GPIOIntRegister(GPIO_PORTE_BASE, KeyReleasedHandler);
    GPIOIntTypeSet(GPIO_PORTE_BASE, Pin, GPIO_FALLING_EDGE);
    GPIOIntClear(GPIO_PORTE_BASE, Pin);
    //Delay(); 
  }
  
  else 
  {
    GPIOIntRegister(GPIO_PORTE_BASE, KeyPressedHandler);
    GPIOIntTypeSet(GPIO_PORTE_BASE, Pin, GPIO_RISING_EDGE);
    GPIOIntClear(GPIO_PORTE_BASE, Pin);
    //Delay();
...
{
  uint32_t ColumnNumber = GPIOIntStatus(GPIO_PORTE_BASE, false);
  canIncrment = true ; 
isPressedFlag = false;
  if (ColumnNumber & GPIO_PIN_1)
      ChangeHandler( GPIO_PIN_1);

  if (ColumnNumber & GPIO_PIN_2)
      ChangeHandler(GPIO_PIN_2);
  
  if (ColumnNumber & GPIO_PIN_3)
      ChangeHandler( GPIO_PIN_3);
  
  if (ColumnNumber & GPIO_PIN_4)
      ChangeHandler(GPIO_PIN_4);
}
...
void KeyPressedHandler()
{
  ...
  canIncrment = false ; 
	...
	if (ColumnNumber & GPIO_PIN_1)
	{
		switch(Module)
		 {
			case Calculator:
				HandleCalculatorInput(4*row+1);
				break;
				
			case RTC:
				HandleRTCInput(4*row+1);
				break;
				
			case List:
				HandleContactListInput(4*row+1);
				break;
  ...
  if (ColumnNumber & GPIO_PIN_2)
	{    
		switch(Module)
		 {
			case Calculator:
				HandleCalculatorInput(4*row+2);
				break;
				
			case RTC:
				HandleRTCInput(4*row+2);
				break;
				
			case List:
				HandleContactListInput(4*row+2);
				break;
  ...
  if (ColumnNumber & GPIO_PIN_3)
  {
	  switch(Module)
		 {
			case Calculator:
				HandleCalculatorInput(4*row+3);
				break;
				
			case RTC:
				HandleRTCInput(4*row+3);
				break;
				
			case List:
				HandleContactListInput(4*row+3);
				break;
  ...
  if (ColumnNumber & GPIO_PIN_4)
  {
	  switch(Module)
		 {
			case Calculator:
				HandleCalculatorInput(4*row+4);
				break;
				
			case RTC:
				HandleRTCInput(4*row+4);
				break;
				
			case List:
				HandleContactListInput(4*row+4);
				break;
